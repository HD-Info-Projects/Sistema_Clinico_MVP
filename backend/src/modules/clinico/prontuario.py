...
from src.integrations.biodata import (
...
def _executar_historico_biodata(where_clause, params, limit, offset):
    return executar_historico_biodata(where_clause, params, limit, offset)
...
def _historico_biodata(paciente_id, usuario_id, limit=10, offset=0):
    ...
    biodata_indisponivel = False
    try:
        historico, has_more = buscar_historico_biodata(cpf, nome, limit, offset)
    except BioDataUnavailableError:
        historico = []
        has_more = False
        biodata_indisponivel = True
    ...
    if biodata_indisponivel:
        response["source_unavailable"] = True
    return response
