...
def test_valores_paciente_spdata_normaliza_campos():
...
def test_buscar_atendimento_existente_usa_janela_do_mesmo_minuto():
    class Cursor:
    ...
    assert cursor.params[3] == datetime(2026, 9, 2, 8, 30)
    assert cursor.params[4] == datetime(2026, 9, 2, 8, 30, 59)
...
def test_salvar_atendimento_nao_insere_quando_ja_existe(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda cursor, paciente_id, id_tbcbo, unidade, data_hora: {
