...
def paciente_para_frontend(row):
    ...
    return {
        ...
        "cpf": normalizar_cpf(row.get("CPF")) or "",
...
def salvar_paciente_spdata(payload):
    ...
    with ConnectionDBFireBird() as connection:
        ...
        if paciente:
            ...
            valores_update.pop("DTCAD", None)
            valores_update.pop("DATA_HORA_INCLUSAO", None)
            valores_update.pop("TIPOPAC", None)
            valores_update.pop("OTMU", None)
            valores_update.pop("NACIONALIDADE", None)
            valores_update.pop("REALIZA_CHAMADO_APELIDO_SOCIAL", None)
...
def buscar_tbcbo_atendimento(cursor, medico_id=None, crm=None):
    filtros = []
    params = []
...
def buscar_medico_payload(cursor, payload):
    ...
    if (not crm and medico_spdata_id is None) and medico_local_id is not None:
    ...
    return buscar_tbcbo_atendimento(cursor, medico_id=medico_spdata_id, crm=crm)
...
def buscar_procedimento_atendimento(cursor, payload):
    ...
    if procedimento_id is None and not codigo:
        return None
...
def buscar_atendimento_existente(cursor, paciente_id, id_tbcbo, unidade, data_hora):
    inicio = data_hora.replace(second=0, microsecond=0)
    fim = inicio + timedelta(seconds=59)
    sql = """
        SELECT FIRST 1
...
def criar_atendimento_firebird(cursor, tabela_colunas, payload, unidade, paciente_id, medico, procedimento, data_hora):
    ...
    valores = {
        ...
        "OBS_ATENDIMENTO": normalizar_texto(payload.get("observacao") or payload.get("obs"), 500),
        "ID_TBCONVEN": normalizar_int(payload.get("idConvenioSpdata") or payload.get("convenioId")),
        "ID_TBCENCUS": normalizar_int(unidade.codigo_spdata_centro_custo),
        ...
        "DATA_HORA_ULTIMA_ATUALIZACAO": agora,
        "ATIVO": "T",
        "CARATER_ATEND": normalizar_int(payload.get("caraterAtendimento")),
    }
    ...
    if novo_id is not None:
        ...
        if "COD_ATENDIMENTO" in tabela_colunas:
            valores_insert["COD_ATENDIMENTO"] = normalizar_int(
                payload.get("codAtendimento") or payload.get("registro"),
            ) or novo_id
...
def salvar_atendimento_spdata(payload, usuario_id, unidade_id=None):
    ...
    if not unidade.codigo_spdata_centro_custo:
    ...
    data_atendimento = normalizar_data(payload.get("data") or payload.get("dataAtendimento"))
    hora_atendimento = normalizar_hora(payload.get("horario") or payload.get("hora"))
